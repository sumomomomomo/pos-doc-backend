...
public class StoredPdfMaterializer {
	...
	public MaterializedPdf materialize(DocumentRenderSource source) {
		...
		try (InputStream in = openStream(source.objectKey())) {
			...
			// Delete the partial temp file on any rendering failure
			// (size mismatch, hash mismatch, storage error, etc.) before
			// rethrowing. The file may have been partially written.
			deleteQuietly(tempFile);
			throw e;
		}
		catch (IOException e) {
