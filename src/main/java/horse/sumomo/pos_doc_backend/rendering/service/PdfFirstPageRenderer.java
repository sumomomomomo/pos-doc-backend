...
public class PdfFirstPageRenderer {
	...
	private RenderedFirstPage doRender(StoredPdfMaterializer.MaterializedPdf pdf, UUID documentId) {
		...
		try {
			...
			// box, rotation, and configured DPI. PDFBox uses Math.round for
			// its internal dimension calculation.
			int pixelWidth = (int) Math.round(widthPoints * this.properties.dpi() / 72.0);
			int pixelHeight = (int) Math.round(heightPoints * this.properties.dpi() / 72.0);
